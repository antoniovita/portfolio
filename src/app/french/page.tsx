    ...
    <div className="min-h-screen bg-transparent text-white flex flex-col relative overflow-auto">
      ...
      <div id="about" className="flex flex-col xl:flex-row sm:flex-col items-center justify-center gap-8 md:gap-20 z-30 h-screen px-4">
        ...
        <motion.div className="mt-8 sm:mt-4 p-4 md:p-20 flex flex-col justify-center space-y-4 max-w-[700px]" initial={{ opacity:0 }} whileInView={{ opacity: 1 }} transition={{ duration: 3, ease: "easeOut" }} viewport={{ amount: 0.2 }}>
          <h1 className="text-3xl md:text-4xl font-semibold text-white text-center md:text-left"> Que dire de <span className="text-yellow-300">moi</span>?</h1>
          <p className="text-md md:text-lg font-light text-gray-300 text-center md:text-left">  Je suis étudiant en informatique à la PUC-Rio avec une bourse de mérite. J'ai 1 an d'expérience en développement web, principalement avec Typescript, React, Node.js et des bases de données comme MongoDB et PostgreSQL. Je m'intéresse également au développement de blockchain et je suis toujours à la recherche de nouvelles connaissances.</p>
