...
const Home = () => {
      ...
      <div className="relative">
        <motion.div initial={{ opacity:0 }} whileInView={{ opacity: 1 }} transition={{ duration: 3, ease: "easeOut" }} viewport={{ amount: 0.2 }} className="relative flex flex-col items-center justify-center z-30 text-white h-screen px-4">
            ...
            <a href="#about" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="bg-black duration-300 all-transition hover:text-white flex flex-col justify-center items-center text-gray-200">
              <p className="font-thin"> Ver Mais </p>
              <svg
  ...
  <div className="flex flex-col justify-center items-center h-screen relative"> 
    <h1 className="text-3xl md:text-4xl z-30 mb-[5px] text-center bg-black rounded-full border border-gray-950 px-5 py-3"> Qual é a minha<span className="text-yellow-300"> stack </span> ?</h1>
    <button onClick={() => setShowStack(true)} className="bg-yellow-400 text-black rounded-full px-3 py-2 mt-5 font-normal hover:bg-yellow-900 hover:text-black all-transition duration-300"> Abrir a lista </button>
  ...
  <motion.div className="sm:mt-4 p-4 sm:p-10 md:p-20 flex flex-col justify-center space-y-4 max-w-[700px]" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 3, ease: "easeOut" }} viewport={{ amount: 0.2 }}>
    ...
    O que dizer sobre <span className="text-yellow-300">mim </span>?
    </h1>
    <p className="text-sm sm:text-md md:text-lg font-light text-gray-300 text-center md:text-left">
    Sou estudante de Ciência da Computação na PUC-Rio com bolsa por mérito. Tenho um ano de experiência em desenvolvimento web, principalmente com TypeScript, React, Node.js e bancos de dados como MongoDB e PostgreSQL. Também tenho interesse em desenvolvimento com blockchain e estou sempre buscando aprender mais.    </p>
